...
function makeUI()
{
    ...
    root.appendChild(p);
    var juma = document.createElement('i');
    p.appendChild(juma);
    juma.appendChild(document.createTextNode('Clika en usuario para generar un ID random'));
    ...
    tdBase64.appendChild(document.createTextNode('Base64 ID (Chiaki)'));
    ...
    for(var i = 0; i < 16; i++)
    {
        ...
        button.appendChild(document.createTextNode('Insertar y Activar'));
...
function fetchAccount(i)
{
    ...
    {
        var juma = document.createElement('i');
        ui[i].username.appendChild(juma);
        juma.appendChild(document.createTextNode('Cuenta Inexistente'));
        ui[i].id.value = '';
...
function activateAccount(idx)
{
    ...
    for(var i = 7; i >= 0; i--)
    {
        ...
        if(digit != digit)
        {
            alert("Debe tener 16 caracteres de largo y solo contenga [0-9A-F]");
            return;
        }
    ...
    if(ui[idx].id.value == acc_id.toUpperCase())
        alert("PSN ID Activada Korrectamente!");
    return false;
}
