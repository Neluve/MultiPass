...
function CalcTime(dur){hrs=Math.floor(dur/1000/60/60);min=Math.floor(dur/1000/60-hrs*60);sec=Math.floor(dur/1000-min*60);mil=dur.toString().slice(-3);if (min!=0){ShowDuration=" - HacKTivated in : "+min+" minute"+(min==1?"":"s")+", "+sec+" Second"+(sec==1?"":"s");}else {ShowDuration=" - HacKTivated in : "+sec+" Second"+(sec==1?"":"s");}}
...
function allset(){
 ...
 msgs.innerHTML="<font color=#73FFFF>PS4 HacKTivated<font color=#FCE4D1> & <font color=#FFB736>Gold<font color=#57E33C>Hen <font color=#73FFFF>Initiated!";
}

function awaitpl() {
 msgs.innerHTML="<font color=#FFB736 size=6>Gold<font color=#57E33C>Hen <font color=#5DEBE5>it's already charged! <font color=#CFF0A1 size=5><br>BinLoader <font color=#B9D9FF>Prepared! <font color=#3EE3F7>Send a Payload to Port <font color=#01F492>9020<font color=#3EE3F7>";
}
