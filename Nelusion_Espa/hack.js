...
function CalcTime(dur){hrs=Math.floor(dur/1000/60/60);min=Math.floor(dur/1000/60-hrs*60);sec=Math.floor(dur/1000-min*60);mil=dur.toString().slice(-3);if (min!=0){ShowDuration=" - HacKTivada en : "+min+" minute"+(min==1?"":"s")+", "+sec+" Segundo"+(sec==1?"":"s");}else {ShowDuration=" - HacKTivada en : "+sec+" Segundo"+(sec==1?"":"s");}}
...
function allset(){
 ...
 msgs.innerHTML="<font color=#73FFFF>PS4 Hacktivada<font color=#FCE4D1> y <font color=#FFB736>Gold<font color=#57E33C>Hen <font color=#73FFFF>Iniciado!";
}

function awaitpl() {
 msgs.innerHTML="<font color=#FFB736 size=6>Gold<font color=#57E33C>Hen <font color=#5DEBE5>ya esta Kargado! <font color=#CFF0A1 size=5><br>BinLoader <font color=#B9D9FF>Preparado! <font color=#3EE3F7>Manda un Payload al Puerto <font color=#01F492>9020<font color=#3EE3F7>";
}
...
function kernelExploit() {
 ...
 var trigger_spray = function () {
  ...
  msgs.innerHTML="⚠️   Konecta el Dispositivo HackTivador USB   ⚠<br>⚠️   Click en Aceptar kuando Desaparezka el Kartel Notificador   ⚠️";
  ...
  alert("Exploit fallido -_- ( Enkabezado de Kernel a Medias...talvez el Pendrive a Destiempo\n\n                    Puedes Probar Reiniciando...");
  p.write8(0, 0);
  return;
